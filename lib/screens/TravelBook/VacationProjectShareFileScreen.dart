...
class VacationProjectShareFileScreen extends StatefulWidget {
  ...
  VacationProjectShareFileScreen({this.title,this.totalKm,this.projectMode,this.pinDestination,this.noOfPerson,this.id,this.start_vacation_date});*/
  Map<String, dynamic> data;
  VacationProjectShareFileScreen({this.data});



  @override
  State<VacationProjectShareFileScreen> createState() => _VacationProjectShareFileScreenState();
}

class _VacationProjectShareFileScreenState extends State<VacationProjectShareFileScreen> {
  ...
  Widget build(BuildContext context) {
            ...
            child: Column(
              children: [
                Stack(
                  children: [
                    MyTopHeaderTheme(
                      headerName:"${widget.data["project_name"].toUpperCase()}\n${widget.data["pin_destination"]!= null ? widget.data["pin_destination"].toUpperCase():""}",
                ...
                MyText(
                  text_name: int.parse(widget.data["mode"]) == ApiParameter.REFLECT_MODE ? "project_release_in_reflect".tr : "project_release_in_inspire".tr,
   ...
   buildPersonCountAndKm() {
    return Row(
              ...
              children: [
                MyTextStart(
                  text_name: "${widget.data["km"]} KM",
                  txtfontsize: MyFontSize.size23,
                  txtcolor: int.parse(widget.data["mode"]) == ApiParameter.REFLECT_MODE ?MyColors.lightGreenColor:MyColors.lineColor,
                  myFont: MyStrings.courier_prime_bold,
                ),
                Container(
  ...
  _columnItem(int index){
    int selectedMode = int.parse(widget.data["mode"]);
  ...
  updateTheme() async {
    ...
    Map<String, String> param = {
      ...
      "userId": MyPreference.getPrefStringValue(key: MyPreference.userId),
      "projectId": widget.data["project_id"].toString(),
    };
