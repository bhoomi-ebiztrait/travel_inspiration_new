...
class VacationProjectFileScreen extends StatefulWidget {
  ...
  @override
  State<VacationProjectFileScreen> createState() =>
      _VacationProjectFileScreenState();
}
...
class _VacationProjectFileScreenState extends State<VacationProjectFileScreen> {
  ...
  void initState() {
    ...
    personCounter =
        myController.allProjectList[widget.projIndex].projectNoPerson != null
            ? int.parse(
                myController.allProjectList[widget.projIndex].projectNoPerson)
            : 0;
    myController.selectedDate.value =
        myController.allProjectList[widget.projIndex].projectVacationDate;
  }
              ...
              children: [
                Stack(
                  children: [
                    MyTopHeaderTheme(
                      headerName:
                          "${myController.allProjectList[widget.projIndex].title.toUpperCase()}\n${myController.allProjectList[widget.projIndex].pinDestination.toUpperCase()}",
                      headerImgUrl: MyImageURL.setting_top,
                      networkImgUrl: myController
                          .allProjectList[widget.projIndex].project_image,
                ...
                MyText(
                  text_name: int.parse(myController
                              .allProjectList[widget.projIndex].projectMode) ==
                          ApiParameter.REFLECT_MODE
                      ? "project_release_in_reflect".tr
                      : "project_release_in_inspire".tr,
...
*/
  ...
  buildsubProj() {
    if (selectProjIndex == -1 &&
        myController.allProjectList[widget.projIndex].pinDestination != null &&
        myController.allProjectList[widget.projIndex].pinDestination != "") {
      if (myController.allProjectList[widget.projIndex].subProjectDetail !=
          null) {
        if(myController.allProjectList[widget.projIndex].subProjectDetail.length > 0) {
          if (myController.allProjectList[widget.projIndex].id !=
              myController.allProjectList[widget.projIndex].subProjectDetail
                  .last.id) {
            SubProjectDetail subDetails = SubProjectDetail();
            subDetails.id = myController.allProjectList[widget.projIndex].id;
            subDetails.name =
                myController.allProjectList[widget.projIndex].pinDestination;
            subDetails.latitude =
                myController.allProjectList[widget.projIndex]
                    .pin_destination_lat;
            subDetails.longitude =
                myController.allProjectList[widget.projIndex]
                    .pin_destination_long;
            myController.allProjectList[widget.projIndex].subProjectDetail
                .add(subDetails);
          }
        }else{
    ...
    if (myController.allProjectList[widget.projIndex].subProjectDetail !=
        null) {
      mheight = ((myController
                  .allProjectList[widget.projIndex].subProjectDetail.length) /
              100) *
          6;
    }
    print("mhe:: $mheight");
    return SizedBox(
      height: Get.height * mheight,
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 10.0, vertical: 1),
        child: GridView.builder(
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: 3, childAspectRatio: 2.5, mainAxisSpacing: 5),
          ...
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              ...
              child: Container(
                child: MyButtonSmall(
                  bgcolor: selectProjIndex == index
                      ? MyColors.lightGreenColor
                      : MyColors.whiteColor,
                  btn_name: myController.allProjectList[widget.projIndex]
                      .subProjectDetail[index].name,
                  txtcolor: MyColors.textColor,
                  onClick: () {
                    setState(() {
                      selectProjIndex = index;
                      myController.selectedPlace.value.lat = double.parse(
                          myController.allProjectList[widget.projIndex]
                              .subProjectDetail[index].latitude);
                      myController.selectedPlace.value.lng = double.parse(
                          myController.allProjectList[widget.projIndex]
                              .subProjectDetail[index].longitude);
                      myController.selectedPlace.value.name = myController
                          .allProjectList[widget.projIndex]
                          .subProjectDetail[index]
                          .name;
                    });
                  },
                ),
    ...
    /* return Padding(
      ...
      child: GridView.count(
        ...
        children: myController
            .allProjectList[widget.projIndex].subProjectDetail
            .map((value) {
          return Padding(
  ...
  buildPersonCountAndKm() {
    return Row(
      ...
      children: [
        MyTextStart(
          text_name:
              "${myController.allProjectList[widget.projIndex].totalKm} KM",
          txtfontsize: MyFontSize.size23,
          txtcolor: int.parse(myController
                      .allProjectList[widget.projIndex].projectMode) ==
                  ApiParameter.REFLECT_MODE
              ? MyColors.lightGreenColor
              : MyColors.lineColor,
  ...
  buildOptions() {
    return Container(
      ...
      child: TIPopupGridviewMenu(
        popupTitle: myController.selectedPlace.value.name != null
            ? myController.selectedPlace.value.name
            : myController.allProjectList[widget.projIndex].title,
        startIndex: 0,
        endIndex: 2,
      ),
    );
  ...
  _columnItem(int index) {
    int selectedMode =
        int.parse(myController.allProjectList[widget.projIndex].projectMode);
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: selectedTransportIndex == index
              ? selectedMode == ApiParameter.REFLECT_MODE
                  ? MyColors.lightGreenColor
                  : MyColors.lineColor
              : MyColors.whiteColor,
        ),
            ...
            children: [
              ...
              InkWell(
                  onTap: () async {
                    // _openGallery();
                    croppedImg =
                        await Get.to(() => PhotoPreviewScreen("gallery".tr));
                    if (croppedImg != null) {
                      print("crop ${croppedImg.path}");
                      updateTheme();
                    } else {
              ...
              InkWell(
                  onTap: () async {
                    croppedImg =
                        await Get.to(() => PhotoPreviewScreen("camera".tr));
                    if (croppedImg != null) {
                      print("crop ${croppedImg.path}");
                      updateTheme();
                    } else {
  ...
  callUpdateProjectApi() async {
    ...
    apiManager.updateProjectApiCall(param: param).then((response) {
      Get.back();
